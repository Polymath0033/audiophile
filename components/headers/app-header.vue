...
<template>
  ...
  <CartModal v-if="_modal" @close="closeModel" />
    <header
      class="flex justify-between w-full py-9 items-center fixed top-0 left-0 px-[140px] bg-black z-10"
    >
      <AppLogo />
      <nav>
        <ul class="flex items-center space-x-10">
          <li>
            <nuxt-link
              to="/"
              class="text-white text-[13px] font-bold !tracking-[2px] !leading-[192.308%] uppercase"
              >Home</nuxt-link
            >
          </li>
          <li>
      ...
      <button v-on:click="toggleModal" type="button">
        <svg
    ...
    <div
      class="w-[calc(100%_-_280px)] left-0 mx-[140px] z-10 flex justify-center items-center h-[1px] bg-white opacity-20 fixed top-[97px]"
    ></div>
</template>
